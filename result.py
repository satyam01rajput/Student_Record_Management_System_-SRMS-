...
class resultClass:
    def __init__(self, root):
        ...
        self.root.geometry("980x500+30+220")
        self.root.config(bg="white")
        self.root.focus_force()

        title = Label(self.root, text="Add Student Result", font=("goudy old style", 20, "bold"), bg="#D4834D", fg="#262626")
        ...
        btn_search = Button(self.root, text="Get", font=("goudy old style", 15, "bold"), bg="#065494", fg="white", cursor="hand2",command=self.search).place(x=500, y=100, width=80, height=30)

        txt_search_name = Entry(self.root, textvariable=self.var_name, font=("goudy old style", 20, 'bold'), bg="lightyellow", state="readonly").place(x=280, y=160, width=300)
        txt_search_course = Entry(self.root, textvariable=self.var_course, font=("goudy old style", 20, 'bold'), bg="lightyellow", state="readonly").place(x=280, y=220, width=300)
        txt_search_marks = Entry(self.root, textvariable=self.var_marks, font=("goudy old style", 20, 'bold'), bg="lightyellow",).place(x=280, y=280, width=300)
        txt_search_full_marks = Entry(self.root, textvariable=self.var_fullmarks, font=("goudy old style", 20, 'bold'), bg="lightyellow",).place(x=280, y=340, width=300)
        ...
        self.bg_img = self.bg_img.resize((620, 250))
        self.bg_img = ImageTk.PhotoImage(self.bg_img)

        self.lbl_bg = Label(self.root, image=self.bg_img).place(x=590, y=70, width=450, height=400)

    # ✅ fetch_roll method correctly placed outside __init__
    def fetch_roll(self):
        ...
        try:
            if self.var_name.get()=="":
                ...
                if row!=None:
                    ...
                    con.commit()
                    messagebox.showinfo("Success","Result Added Successfullly",parent=self.root)            
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to {str(ex)}")
